...
CONFIG_PATH = os.path.join(BASE_DIR, '/home/TableON/config/config.json')
RECIPE_PATH = os.path.join(BASE_DIR, '/home/TableON/config/recipe.json')
...
def get_valid_menu_codes():
    """레시피 파일에서 판매 가능한 메뉴 코드 목록을 가져옵니다."""
    try:
        with open(RECIPE_PATH, 'r', encoding='utf-8') as f:
            return {r['menu_code'] for r in json.load(f)}
    except Exception as e:
...
def fetch_and_process_orders(db_config, valid_menu_codes):
    """DB에서 신규 주문을 가져와 서버로 전송하는 메인 로직."""
                ...
                for bill in new_bills:
                    ...
                    for detail in cur.fetchall():
                        ...
                        if menu_code in valid_menu_codes:
                            print(f"[Order] 신규 주문 발견: 주문번호={order_no}, 메뉴={menu_code}, 수량={quantity}")
...
if __name__ == "__main__":
    ...
    while True:
        try:
            valid_menu_codes = get_valid_menu_codes()
            if valid_menu_codes:
                fetch_and_process_orders(db_config, valid_menu_codes)
            else:
                print("[WARN] 유효한 메뉴 코드를 찾을 수 없습니다. 10초 후 재시도합니다.")
                time.sleep(10)
